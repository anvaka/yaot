...
TreeNode.prototype.query = function queryBounds(results, sourceArray, intersects) {
  ...
  if (items) {
    for (var i = 0; i < items.length; ++i) {
      ...
      var z = sourceArray[idx + 2];
      if (intersects.precise(x, y, z)) {
        results.push(idx);
      }
    }
  }
  ...
  this.q0.query(results, sourceArray, intersects);
  this.q1.query(results, sourceArray, intersects);
  this.q2.query(results, sourceArray, intersects);
  this.q3.query(results, sourceArray, intersects);
  this.q4.query(results, sourceArray, intersects);
  this.q5.query(results, sourceArray, intersects);
  this.q6.query(results, sourceArray, intersects);
  this.q7.query(results, sourceArray, intersects);
};
